...
def backward(a, b, c, d, e, fa, fb, fc, fd, fe, xi, excel, check):
    answer = list()
    flag = False
    if e != '':
        try:
    ...
    # 4th Order:
    if flag:
        f4 = func4(e, d, c, b, a, fe, fd, fc, fb, fa)
    ...
    if check:
        ...
        worksheet.write(row + 6, col + 1, fd, cell_format)
        ...
        if flag:
            ...
            worksheet.write(row + 8, col + 1, fe, cell_format)
            worksheet.write(row + 5, col + 2, func(c, d, fc, fd))
            worksheet.write(row + 7, col + 2, func(d, e, fd, fe), cell_format)
            worksheet.write(row + 4, col + 3, func2(b, c, d, fb, fc, fd))
            worksheet.write(row + 6, col + 3, func2(c, d, e, fc, fd, fe), cell_format)
